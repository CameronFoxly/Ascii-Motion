 ...
 * - Consistent UI patterns following CharacterMappingSection
 */
...
export function TextColorMappingSection({ onSettingsChange }: TextColorMappingSectionProps) {
  ...
  const handleQuantizationChange = (value: string) => {
    ...
    updateSettings({ textColorQuantization: quantization });
    onSettingsChange?.();
  };
          ...
          {!enableTextColorMapping && (
            <div className="p-3 border border-border/50 rounded-lg bg-muted/20">
              <p className="text-xs text-muted-foreground text-center">
                Text color mapping is disabled. Characters will use the active color from the main editor.
              </p>
            </div>
          )}
          ...
          {enableTextColorMapping && (
            <Card className="bg-card/50 border-border/50 overflow-hidden w-full">
              <CardContent className="p-3 space-y-3 w-full">
                ...
                {/* Header */}
                <div className="flex items-center gap-2">
                  <Label className="text-sm font-medium">Text Color Palette</Label>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="bg-background/50 border border-border rounded p-2 min-h-[40px]">
                      <div className="flex flex-wrap gap-1">
                        {selectedPalette.colors.slice(0, textColorQuantization).map((color) => (
                          <div
                            key={color.id}
                        ...
                        {selectedPalette.colors.length > textColorQuantization && (
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select 
                      value={textColorQuantization.toString()} 
                ...
                <div className="text-xs text-muted-foreground bg-muted/20 p-2 rounded">
                  <p>Maps image colors to the selected palette for ASCII character colors. Uses brightness-based color matching.</p>
                </div>
